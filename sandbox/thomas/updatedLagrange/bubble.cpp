...
int main( int argc, char* argv[] )
{
    ...
    Fluid fluid1( mesh, ui.viscosity1, ui.alpha, stokesStabil, dynamic, ui.rho1, ui.dt  );
    Fluid fluid2( mesh, ui.viscosity2, ui.alpha, stokesStabil, dynamic, ui.rho2, ui.dt  );
    ...
    double prevEnclosed;
    {
        ...
        bubbleFeatures( ffi, fluid1, surfaceQuadrature, cutQuadrature1, std::cout );
        std::cout << std::endl;

        // store the contained volume
        prevEnclosed =
            surf::enclosedVolume<FFI::SU1U1>( surfaceQuadrature, ffi.getBinder() );
    ...
    for ( unsigned step = 0; step < ui.numLoadSteps; step++ ) {
        ...
        fluid1.assembleBulk( cutQuadrature1, solver );

        fluid1.bodyForce( cutQuadrature1, solver,
                          boost::bind( gravity<dim>, _1, ui.rho1 ) );
            
        fluid2.assembleBulk( cutQuadrature2, solver );

        fluid2.bodyForce( cutQuadrature2, solver,
                          boost::bind( gravity<dim>, _1, ui.rho2 ) );
        ...
        // 2) Geometry update
        {
            ...
            const TA::VecDim moment =
                surf::enclosedVolumeMoment<FFI::SU1U1>( surfaceQuadrature, ffi.getBinder() );

...
namespace base{
    namespace post{
        ...
        bool findLocationInMesh( const MESH& mesh,
 ...
 *  \param[out] result    Pair of an element ID and the local coordinate
 *  \return               Flag if location has been succesfully found
 */
template<typename MESH>
bool base::post::findLocationInMesh( const MESH& mesh,
                                     const typename MESH::Node::VecDim& x,
                                     const double tolerance, const unsigned maxIter,
                                     std::pair<std::size_t,
                                     typename MESH::Element::GeomFun::VecDim>& result )
{
    ...
    for ( ; fIter != fLast; ++fIter ) {
        ...
        // in case of success return element ID and local coordinate
        if ( trial.second ) {
            result = std::make_pair( (fIter -> second) -> getID(), trial.first );
            return true;
        }
    }

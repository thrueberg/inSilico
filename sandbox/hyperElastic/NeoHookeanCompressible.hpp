...
public:
    ...
    //--------------------------------------------------------------------------
    /** The elasticity tensor in mixed description.
     *  \f[
     *       C_{ijKL} = \frac{1}{J} \left(
     *         (\lambda-\log J + \mu) \delta_{ij} \delta_{KL} +
     *         \mu ( F_{iL} F_{jK} + F_{iK} F_{jL} - F_{iM} F_{jM} \delta_{KL} )
     *         \right)
     *  \f]
     *  Note: The storage is based on the minor symmetries.
     *  \param[in]  F    Deformation gradient
     *  \param[out] elC  Elasticity tensor
     */
    void mixedElasticityTensor( const Tensor& F, ElastTensor& elC ) const
    {
        // compute determinant of F 
        const double J = mat::determinant( F );
        mat::Tensor b;
        ...
        // factors
        const double fac1 = (lambda_ - lambda_ * std::log(J) + mu_) / J;
        const double fac2 = mu_ / J;
            ...
            for ( unsigned j = i; j < 3; j++ ) {
                ...
                for ( unsigned K = 0; K < 3; K++ ) {
                    for ( unsigned L = K; L < 3; L++ ) {
                        ...
                        // compute C_{ijkl}
                        const double cEntry =
                            fac1 * (i == j ? 1. : 0.) * (K == L ? 1. : 0.) +
                            fac2 * ( F(i,L) * F(j,K) + F(i,K) * F(j,L) -
                                     (K == L ? b(i,j) : 0. ) );

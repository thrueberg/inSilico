...
struct base::MultiIndex
{
    ...
    static std::size_t length( const Type & mi )
    {
        ...
        for ( unsigned s = 0; s < size; s++ ) {
            assert( mi[s] > 0 );
            result *= static_cast<std::size_t>( mi[s] );
        }
        return result;
    }
    ...
    static std::size_t unwrap( const Type & mi, const Type & dimensions )
    {
        ...
        for ( unsigned s = 0; s < size; s++ ) {
            ...
            result += static_cast<std::size_t>( mi[s] ) * stride;
            stride *= static_cast<std::size_t>( dimensions[s] );
        }
        return result;
    }
    ...
    static Type wrap( const std::size_t unwrapped, const Type & dimensions )
    {
        ...
        int aux = static_cast<int>( unwrapped );

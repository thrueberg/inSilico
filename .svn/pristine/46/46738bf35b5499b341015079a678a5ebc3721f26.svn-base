...
template<template<unsigned,int> class FUN, unsigned DEGREE, int CTR>
struct EvaluateRegularity
{
    static void apply( const unsigned derivative,
                       const int continuity, const unsigned resolution,
                       std::ostream& out )
    {
        ...
        if ( continuity == CTR )
            ...
            EvaluateRegularity<FUN,DEGREE,CTR-1>::apply( derivative, continuity,
                                                         resolution, out );
    }
};
...
template<template<unsigned,int> class FUN, unsigned DEGREE>
struct EvaluateRegularity<FUN,DEGREE,-2>
{
    static void apply( const unsigned derivative,
                       const int      continuity, const unsigned resolution,
                       std::ostream& out )
    {
        std::cerr << "Continuity = " << continuity << ", degree = " << DEGREE << std::endl;
...
template<template<unsigned,int> class FUN, int CTR>
struct EvaluateDegrees
{
                       ...
                       std::ostream& out )
    {
        // Go through available degrees
        if ( degree == CTR )
            EvaluateRegularity<FUN,CTR,CTR-1>::apply( derivative, continuity,
                                                      resolution, out );
        else
            EvaluateDegrees<FUN,CTR-1>::apply( degree, derivative,
                                               continuity, resolution, out );
    }
};

//------------------------------------------------------------------------------
// Error dummy not to be called
template<template<unsigned,int> class FUN>
...
int main( int argc, char* argv[] )
{
    if ( argc != 5 ) {
        std::cout << "Usage:  " << argv[0]
                  << " degree  continuity derivative resolution \n";
        return 0;
    }

    // read from user input
    const unsigned degree     = boost::lexical_cast<unsigned>( argv[1] );
    const int      continuity = boost::lexical_cast<unsigned>( argv[2] );

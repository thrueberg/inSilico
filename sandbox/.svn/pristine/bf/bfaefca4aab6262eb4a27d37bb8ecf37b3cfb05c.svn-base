...
//------------------------------------------------------------------------------
template<typename MESH, unsigned DEGU, unsigned DEGP, bool STOKESSTABIL>
class Fluid
{
public:
    //! Template parameter
    typedef MESH Mesh;
    static const unsigned degreeU      = DEGU;
    static const unsigned degreeP      = DEGP;
    ...
    typedef ::Convection<typename UU::Tuple>               Convection;
    ...
    void registerInSolver( SOLVER& solver )
    {
        ...
        if ( stokesStabil_ ) solver.template registerFields<PP>( fb );
    }

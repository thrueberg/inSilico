...
std::istream& operator >>(std::istream &is, BC& bc )
{
    ...
    else if ( buffer == "OPENLR"    ) bc = OPENLR;
                ...
                const base::auxi::BoundingBox<DIM>& bbox ) 
{
    ...
    for ( unsigned d = 0; d < DIM; d++ ) {
        if ( doFPtr -> isActive(d) ) {
            ...
            const typename base::auxi::BoundingBox<DIM>::VecDim xi =
...
void tankP( const typename base::Vector<DIM>::Type& x,
            DOF* doFPtr, const base::auxi::BoundingBox<DIM>& bbox  ) 
{
    const bool top = bbox.isOnUpperBoundary( x, DIM-1, coordTol );
...
//------------------------------------------------------------------------------
/**  Open boundaries on left and right sides.
 *
 *                   closed
 *               o-----------o
 *               ¦           ¦
 ...
 */
template<unsigned DIM, typename DOF>
void openLR( const typename base::Vector<DIM>::Type& x,
             DOF* doFPtr, const double factor,
             const base::auxi::BoundingBox<DIM>& bbox  ) 
{
    const bool left  = bbox.isOnUpperBoundary( x, 0, coordTol );
    const bool right = bbox.isOnLowerBoundary( x, 0, coordTol );
...
void dirichletBC( const typename base::Vector<DIM>::Type& x,
                  DOF* doFPtr, const double factor, 
                  const base::auxi::BoundingBox<DIM>& bbox, const BC& bc )
{
    if ( bc == SHEAR )
        ...
        return openLR<DIM,DOF>( x, doFPtr, factor,   bbox  );
...
//==============================================================================
template<unsigned DIR, unsigned DIM>
typename base::Vector<DIM>::Type
unitForce( const typename base::Vector<DIM>::Type& x,
           const double value )
{
